...
> 这是一个 **V0.1 MVP**。完整链路已经跑通，并且已经部署上线，可以直接打开体验（见下方「在线 Demo」），电脑端与手机端均已实际验证可用。
>
> 当前仍是单机自用级别的实现：没有账号体系、没有数据库、不做自动发布（详见 [当前限制](#11-当前限制)）。
...
无需安装、无需配置 API Key，打开即可体验完整链路：上传 → 分析 → 选题 → 标题 → 正文与标签 → 发布预览 → 复制。

- 在线 Demo 已部署（平台：Railway）
- 电脑端已验证
- 手机端已验证

想从源码运行、改代码或跑测试，请看 [本地运行](#7-本地运行)。
...
| 方式 | 适合谁 | 需要做什么 |
| --- | --- | --- |
| 在线体验 | 只想看效果 | 直接打开线上 Demo，无需本地运行 |
| 本地开发 | 想读源码 / 改代码 / 跑测试 | `npm install` → 配置 `.env.local` → `npm run dev` → 访问 `http://localhost:3000` |
...
不需要安装 Node.js，也不需要配置任何 API Key。
...
**平台：** Railway　**框架：** Next.js　**当前版本：** V0.1
...
| 框架 | Next.js |
| 当前版本 | V0.1 |
| 在线 Demo | https://doutu-ai-production.up.railway.app/ |

- 在线 Demo 已部署
- 电脑端已验证
- 手机端已验证

模型密钥通过 Railway 的环境变量注入，变量名与本地 `.env.local` 一致（见 [AI 模型配置](#6-ai-模型配置)），仅存在服务端，不写入仓库。
...
| 电脑端 | 已验证 |
| 手机端 | 已验证 |
| 测试 | 314 tests passed（17 test files） |
| 核心工作流 | 已完成 |
